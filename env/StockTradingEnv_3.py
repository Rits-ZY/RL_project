...
class StockTradingEnv(gym.Env):
    ...
    def __init__(self, df):
        ...
        self.action_space = spaces.Box(low=np.array(
            [-0.2, 0.1, 1, 0]), high=np.array([0.2, 0.6, 5, 1]), dtype=np.float16)
    ...
    def step(self, action):
        volume, dw, cdSec, all_in= action  # volume 量  dw ？ cdsec 持仓时间
        self.act = action
        cdSec = float(cdSec)
        ...
        if all_in <=0.8:
            ...
            if self.base != 0:
                try:
                    askOrderPrice = np.array(ap)[av.cumsum() > self.base][0]
                except BaseException:
                    # print(np.array(ap))
                    # print("*********")
                    # print(np.array(av))
                    ...
                    askOrderPrice = None
                if askOrderPrice:
                    self.quote = self.quote + 0.999*self.base * askOrderPrice
                    self.base = 0
                    self.curIdx = self.curIdx + math.ceil(cdSec)
                else:
                    self.curIdx = self.curIdx + math.ceil(cdSec)
        ...
        elif self.curIdx > 300 and self.fill_count < 2:
            reward = -3
            self.sharp = -3
        else:
            reward = 0
